...
function loginUser(req, res) {
  passport.authenticate('local', (err, user) => {
    if (err || !user) {
      return res.status(500).json({ message: 'Registration successful, but login failed', error: err });
    }
    ...
    req.logIn(user, async (err) => {
      ...
      try {
        ...
        return res.status(500).json({ message: 'Registration successful, but unable to retrieve user details', error: error });
      }
    });
  })(req, res);
}
